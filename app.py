...
def alpha_blend(frames, alphas):
    ...
    for f,a in zip(frames, alphas): out += f.astype(np.float32) * a
...
def line_from_canvas(json_data):
    if not json_data or "objects" not in json_data: return None
...
def trim_clip(src_path, dst_path, t0, t1):
    # moviepy nutzt automatisch imageio-ffmpeg (kein System-ffmpeg nötig)
    with VideoFileClip(src_path) as clip:
        sub = clip.subclip(max(t0,0), max(t1,0))
        sub.write_videofile(dst_path, codec="libx264", audio=False, fps=clip.fps, verbose=False, logger=None)
...
def detect_times(video_path, entry_line, exit_line, model):
    ...
    if not ok: return [], fps, first
    width_ref = first.shape[1]

    stream = model.track(source=video_path, stream=True, tracker="bytetrack.yaml",
                         classes=[2,3,5,7], conf=0.25, verbose=False)  # car, moto, bus, truck
    frame_i = 0
    for r in stream:
    ...
    valid.sort(key=lambda x:x[3])  # by sector time
    return valid, fps, first

# ----------------- UI -----------------
st.set_page_config(page_title="S-Curve Analyzer (Web)", layout="wide")
st.title("S-Curve Analyzer – komplett im Browser (kostenlos)")
...
if uploaded:
    ...
    with c1:
        entry = st_canvas(
            ...
            background_image=cv2.cvtColor(first_frame, cv2.COLOR_BGR2RGB),
            height=first_frame.shape[0], width=first_frame.shape[1],
            drawing_mode="line", key="entry",
        )
    with c2:
        exitc = st_canvas(
    ...
    if st.button("Analysieren"):
        ...
        with st.spinner("Analysiere Clips …"):
            for idx, vpath in enumerate(tmp_paths):
                ...
                if not valid:
                    results_table.append({"Clip": os.path.basename(vpath),
        ...
        sel = st.multiselect("Wähle Clips", options=list(range(len(names))),
                             default=list(range(min(3,len(names)))))
        if sel:
            chosen = [trimmed[i] for i in sel][:3]
            # Overlay bauen (einfaches Alpha-Blend, gleiche Breite, kein Homography-Fein-Align in Web-Variante)
